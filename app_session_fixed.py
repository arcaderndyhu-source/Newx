...
def build_mobile_headers(profile: dict, csrf_token: str = "", session=None) -> dict:
    ...
    # IMPORTANT: do not freeze the Cookie header here. Requests.Session already
    # manages its cookie jar and can accept rotated Set-Cookie values from Instagram.
    # A manually-built Cookie header can otherwise keep stale session/csrf values.
    return headers
...
def verify_session(session: requests.Session, headers: dict) -> dict:
    ...
    """
    csrf_token = get_cookie(session, "csrftoken", "") or headers.get("X-CSRFToken", "")
    if csrf_token:
        headers["X-CSRFToken"] = csrf_token
...
def _sync_live_instagram_headers(session, headers):
    """Keep auth headers aligned with the current Requests cookie jar."""
    try:
        csrf = get_cookie(session, "csrftoken", "")
        if csrf:
            headers["X-CSRFToken"] = csrf
    except Exception:
        pass

def _session_response_sync(response, *args, **kwargs):
    """Requests response hook: refresh CSRF/header state after Set-Cookie updates."""
    try:
        sess = getattr(response, "connection", None)
        # The hook is attached with a closure below; this fallback intentionally does nothing.
    except Exception:
        pass
    return response

def _install_session_sync_hook(session, headers):
    """Install a lightweight hook that copies rotated CSRF state into headers."""
...
def init_account_session(uid, sessionid, csrf_token="", username=""):
    ...
    _install_session_sync_hook(s, headers)
    ...
    # Sync any CSRF value received during client setup before verification.
    _sync_live_instagram_headers(s, headers)
