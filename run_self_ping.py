...
# Optional Render/self-ping configuration.
# Priority: SELF_URL -> RENDER_EXTERNAL_URL -> PUBLIC_URL.
SELF_URL = os.environ.get("SELF_URL", "").strip()
if not SELF_URL:
    SELF_URL = os.environ.get("RENDER_EXTERNAL_URL", "").strip()
if not SELF_URL:
    SELF_URL = os.environ.get("PUBLIC_URL", "").strip()
SELF_PING_INTERVAL = max(15, int(os.environ.get("SELF_PING_INTERVAL", "45")))

def _load_env_value(name: str):
    """Read a simple KEY=value from the local .env without overriding OS env."""
...
def start_self_ping():
    """Continuously ping the configured public URL in a daemon thread."""
    url = SELF_URL or _load_env_value("SELF_URL")
    if not url:
        url = _load_env_value("RENDER_EXTERNAL_URL") or _load_env_value("PUBLIC_URL")
    if not url:
        print("[*] Self-ping disabled: SELF_URL/RENDER_EXTERNAL_URL/PUBLIC_URL not set.")
        return

    url = url.rstrip("/")
    # Ping /health when configured; otherwise ping the base URL.
    path = os.environ.get("SELF_PING_PATH", "/health").strip() or "/health"
    ...
    def _worker():
        ...
        while True:
            try:
                ...
                print(f"[SELF PING] {ping_url} -> HTTP {response.status_code}")
            except Exception as exc:
                print(f"[SELF PING] failed: {exc}")
            time.sleep(SELF_PING_INTERVAL)

    thread = threading.Thread(target=_worker, name="self-ping", daemon=True)
    thread.start()
